...
//	ОткрытьДляПереноса	- Булево 		   - открыть форму для переноса значений из одной колонки в другую
...
//
Процедура ОткрытьФормуЗаполненияВыделенныхСтрок(Форма, знач ОткрытьДляПереноса = Ложь) Экспорт
	...
	ПараметрыФормы = Новый Структура("МассивДанныхТаблицы, ДанныеТекущейСтроки, ОткрытьДляПереноса, Источник", 
		МассивДанныхТаблицы, ДанныеТекущейСтроки, ОткрытьДляПереноса, Форма.УникальныйИдентификатор);
...
Процедура ОбработкаОповещения(Форма, ПараметрыОповещения, знач ОповещениеПриРедактировании = Неопределено, знач ОповещениеПриРедактированииОкончание = Неопределено) Экспорт
	...
	РежимПереноса = ПараметрыОповещения.Свойство("КолонкаПереносНа");
	...
	Для Каждого ЭлементМассива Из ПараметрыОповещения.МассивВыделенныхСтрок Цикл
		...
		КолонкаЗначение 			= СтрокаТаблицы[ПараметрыОповещения.Колонка];
		КолонкаПереносНаЗначение 	= ?(РежимПереноса, СтрокаТаблицы[ПараметрыОповещения.КолонкаПереносНа], Неопределено);
			
		// Вызываем процедуру Формы "ПередИзменениемСтроки"
		Отказ = Ложь;
		
		Попытка
			Форма.Подключаемый_ГрупповоеРедактированиеПередИзменениемСтроки(СтрокаТаблицы, ПараметрыОповещения.Колонка, КолонкаЗначение, 
					?(РежимПереноса, КолонкаПереносНаЗначение, ПараметрыОповещения.ЗначениеДляЗаполнения), Отказ);
		Исключение
			Продолжить;
		КонецПопытки;
		...
		СтруктураРезультат.Вставить("СтрокаТаблицы",	СтрокаТаблицы);
		СтруктураРезультат.Вставить("Колонка",			ПараметрыОповещения.Колонка);
		
		Если НЕ РежимПереноса Тогда
			СтрокаТаблицы[ПараметрыОповещения.Колонка] = ПараметрыОповещения.ЗначениеДляЗаполнения;
			СтруктураРезультат.Вставить("ЗначениеДляЗаполнения", ПараметрыОповещения.ЗначениеДляЗаполнения);
		Иначе
			Если ПараметрыОповещения.Колонка <> ПараметрыОповещения.КолонкаПереносНа Тогда
				СтрокаТаблицы[ПараметрыОповещения.КолонкаПереносНа] = ?(ТипЗнч(КолонкаПереносНаЗначение) = Тип("Число"), 
					КолонкаПереносНаЗначение + СтрокаТаблицы[ПараметрыОповещения.Колонка], СтрокаТаблицы[ПараметрыОповещения.Колонка]);
				СтрокаТаблицы[ПараметрыОповещения.Колонка] = Неопределено;	
			КонецЕсли;
			
			СтруктураРезультат.Вставить("КолонкаПереносНа", ПараметрыОповещения.КолонкаПереносНа);
		КонецЕсли;
