...
// Устанавливает оформление табличной части с формулами
//	1. Предопределенный выделяется жирным
//	2. Элемент с представлением расчета Шрифт 9+наклонный, Цвет текста Серый
//	3. Для реквизита с формулой, сумма только просмотр
//
// Параметры:
//	Форма			- ФормаКлиентскогоПриложения
//	ИмяРеквизита	- Строка - полный путь к реквизиту таб. части формул
//	ИмяЭлемента		- Строка - имя элемента на форме
//		Если не указан, используется имя реквизита (последняя часть после точки)
//
Процедура ТаблицаФормулУстановитьОформление(Форма, знач ИмяРеквизита, знач ИмяЭлемента = "") Экспорт
	
	Если НЕ ЗначениеЗаполнено(ИмяЭлемента) Тогда
		ИмяЭлемента = ПолучитьИмяЭлементаИзРеквизита(ИмяРеквизита);
	КонецЕсли;
	...
	// установим представление расчета в ТолькоПросмотр
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы,
		ИмяЭлемента + "РасчетПредставление", "ТолькоПросмотр", Истина);
	
	Шрифт_Ж = Новый Шрифт(Форма.Элементы[ИмяЭлемента].Шрифт,,, Истина);
	Шрифт_9 = Новый Шрифт(Форма.Элементы[ИмяЭлемента].Шрифт,, 9,, Истина);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементУО.Отбор,
		ИмяРеквизита + ".Предопределенный", Истина, ВидСравненияКомпоновкиДанных.Равно,, Истина);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Шрифт", Шрифт_Ж);
	
	РаботаСФормамиПТБКлиентСервер.ДобавитьПоляУсловногоОформления(ЭлементУО.Поля, ИмяЭлемента);
	#КонецОбласти

	#Область РасчетПредставление
	ЭлементУО = Форма.УсловноеОформление.Элементы.Добавить();
	ЭлементУО.Использование = Истина;
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементУО.Отбор,
		ИмяРеквизита + ".РасчетПредставление", Неопределено, ВидСравненияКомпоновкиДанных.Заполнено,, Истина);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Шрифт", Шрифт_9);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.Серый);
	
	РаботаСФормамиПТБКлиентСервер.ДобавитьПоляУсловногоОформления(ЭлементУО.Поля, ИмяЭлемента + "РасчетПредставление");
	#КонецОбласти
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементУО.Отбор,
		ИмяРеквизита + ".СпособРасчета",
		ПредопределенноеЗначение("Перечисление.СпособыРасчетаПоказателяФормулы.РучнойВвод"),
		ВидСравненияКомпоновкиДанных.НеРавно,,
		Истина);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	РаботаСФормамиПТБКлиентСервер.ДобавитьПоляУсловногоОформления(ЭлементУО.Поля, ИмяЭлемента + "Сумма");
	#КонецОбласти
	
КонецПроцедуры

// Выполняет заполнение существующей (или новой) строки формулы по переданным данным
//
// Параметры:
//	ТабличнаяЧасть		- ТаблицаЗначений, ТабличнаяЧасть, ДанныеФормыКоллекция - табличная часть для заполнения
//		колонки см. Обработка.РаботаСФормулами.ФормулыРасчета
//	Идентификатор		- Строка - идентификатор формулы
//	Колонка				- Строка - имя колонки
//	Представление		- Строка - представление строки формулы
//	ФормулаЗначение		- Число, Строка, Неопределено - значение формулы или ручного значения
//		Если передано значение с типом Число, тогда способ расчета = РучнойВвод, иначе Формула
//	ИмяПоказателя		- Строка - имя показателя, если не установлено, параметр ОписаниеПоказателя игнорируется
//	ОписаниеПоказателя	- Строка - описание показателя, устанавливается если задано ИмяПоказателя
//	НеОкруглять			- Булево - признак отсутствия необходимости округления
//	Точность			- Число - точность округления (если НеОкруглять = Ложь)
//	НеОтрицательное		- Булево - признак положительного значения результата
//	Предопределенный	- Булево - показатель является предопределенным
//
// Возвращаемое значение:
//	СтрокаТаблицыЗначений, СтрокаТабличнойЧасти, ЭлементДанныеФормыКоллекция
//
Функция ТаблицаФормулДобавитьСтроку(ТабличнаяЧасть, знач Идентификатор, знач Колонка, знач Представление,
	знач ФормулаЗначение = Неопределено, знач ИмяПоказателя = "", знач ОписаниеПоказателя = "",
	знач НеОкруглять = Истина, знач Точность = 2, знач НеОтрицательное = Истина, 
	знач Предопределенный = Ложь) Экспорт

	СпособРучнойВвод	= ПредопределенноеЗначение("Перечисление.СпособыРасчетаПоказателяФормулы.РучнойВвод");
	СпособФормула		= ПредопределенноеЗначение("Перечисление.СпособыРасчетаПоказателяФормулы.Формула");
	
	СпособРасчета = ?(ТипЗнч(ФормулаЗначение) = Тип("Число"), СпособРучнойВвод, СпособФормула);
	
	МассивСтрокФормул = ТабличнаяЧасть.НайтиСтроки(Новый Структура("Идентификатор,Колонка", Идентификатор, Колонка));
	...
	Если СтрокаТаблицы.СпособРасчета = СпособРучнойВвод Тогда
		СтрокаТаблицы.РучноеЗначение = ФормулаЗначение;
	ИначеЕсли СтрокаТаблицы.СпособРасчета = СпособФормула Тогда
	...
	Если НЕ ПустаяСтрока(ИмяПоказателя) Тогда
		...
		СтрокаТаблицы.ОписаниеПоказателя	= ОписаниеПоказателя;
	КонецЕсли;
...
// Выполняет пересчет таблицы формул
//
// Параметры:
//	Форма			- ФормаКлиентскогоПриложения
//	ИмяРеквизита	- Строка - полный путь к реквизиту таб. части формул
//	Показатели		- Соответствие - значения показателей для передачи в расчет
//
// Возвращаемое значение:
//   Булево. Ложь - ошибка, Истина - успешно
//
Функция ТаблицаФормулВыполнитьПересчет(Форма, знач ИмяРеквизита, знач Показатели = Неопределено) Экспорт
	ТабличнаяЧасть		= ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, ИмяРеквизита);
	ДанныеДляРасчета	= РаботаСФормуламиКлиентСервер.ПодготовитьДанныеДляРасчета(ТабличнаяЧасть);
...
Процедура СообщитьРезультатРасчета(знач РезультатРасчета, Отказ = Ложь) Экспорт
	...
	Для Каждого СтрокаПротокола Из РезультатРасчета.Протокол Цикл
		ТекстОшибки = СтрШаблон(НСтр("ru='%1 (идентификатор: %2, колонка: %3)'"),
			СтрокаПротокола.Текст,
			СтрокаПротокола.Идентификатор,
			СтрокаПротокола.Колонка);
...
Функция ПолучитьИмяЭлементаИзРеквизита(знач ИмяРеквизита) Экспорт
	Подстроки = СтрРазделить(ИмяРеквизита, ".");
	Если Подстроки.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат СокрЛП(Подстроки[Подстроки.Количество()-1]);
КонецФункции
...
Функция ВыполнитьРасчетКлюча(знач Настройки, знач КлючФормулы)
	...
	Попытка
		...
		|	ТекущееЗначение = " + ?(НЕ ЗначениеЗаполнено(СлужебныеДанные.Формула), "0", СлужебныеДанные.Формула) + ";";
	Исключение
